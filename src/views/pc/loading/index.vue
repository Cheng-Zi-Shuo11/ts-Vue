...
export default class Loadding extends Vue {
  ...
  private loading() {
    let canvas = document.getElementById("loading") as HTMLCanvasElement;
    ...
    window.onmousemove = function(e: any) {
      ...
      // 值越接近-1，表示方向越向左
      maxspeedx =
        (e.clientX - canvas.clientWidth / 2) / (canvas.clientWidth / 2);
    };

    // 根据参数，返回一个rgb颜色，用于给雨滴设置颜色
    function getRgb(r: any, g: any, b: any) {
      return "rgb(" + r + "," + g + "," + b + ")";
    }
    ...
    function createDrop(x: any, y: any) {
      // 一个 drop 对象，代表一个圆弧
      var drop = {
        // 判断是否删除，值为true就删除
        die: false,
    ...
    // 画一定数量的小水珠
    function madedrops(x: any, y: any) {
      // 随机生成一个数 maxi
      // maxi 代表要画小水珠的数量
    ...
    function update() {
      ...
      var endLine =
        canvas.clientHeight - (Math.random() * canvas.clientHeight) / 5;
