...
<template>
  <div id="user_setting" ref="setting">
    <el-card class="box-card" shadow="hover">
      ...
      >
        ...
        <el-form-item>
          ...
          <el-button type="primary" @click="saveImg">截图</el-button>
        </el-form-item>
      </el-form>
    </el-card>
  </div>
...
export default class user extends Vue {
  ...
  private judgeUserInformation() {
    if (window.localStorage.getItem("用户信息")) {
      let form = window.localStorage.getItem("用户信息");
      form = JSON.parse(form as any);
      (this.ruleForm as any) = form;
    }
  }

  // 截图
  private saveImg() {
    this.$html2canvas(this.$refs.setting as any, { useCORS: true }).then(
      function(canvas: any) {
        //获取年月日作为文件名
        var timers = new Date();
